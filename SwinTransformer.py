...
# cite https://github.com/WangFeng18/Swin-Transformer
# In[1]:
...
class EllipseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.tensor(data/255.0, dtype=torch.float), labels
    def __len__(self):
        return 10000
...
class WMSA(nn.Module):
    ...
    def relative_embedding(self):
        cord = torch.tensor(np.array([[i, j] for i in range(self.window_size) for j in range(self.window_size)])).type(torch.long)
...
model = Swin_T(5, input_resolution=224)
model.to(device)
print()
